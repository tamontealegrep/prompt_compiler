...
|---|---|---|---|
| ~~B1~~ | N/A — **by design, not a bug** | `templates/system_prompt.md.j2` (line 239), `templates/system_prompt_bilingual_text.md.j2` (line 244) | ~~Originally flagged as an accidental duplicate render of `[[ input_variables_block ]]`~~. Confirmed with the repo owner (2026-08-06) that the second occurrence is intentional, not a leftover copy-paste. Left as-is — do not "fix" this. |
| ~~B2~~ | N/A — **fixed 2026-08-06** | `app/schemas.py::FlowObjectBase.validate_semantics()` | ~~Per-node-type invariants were incomplete/asymmetric~~ — closed by the invariant matrix in §13's 2026-08-06 decision log entry: `execute` is now forbidden outside `action`/`terminal`; `final='yes'` is now forbidden outside `terminal`; `say` is now forbidden for `action`/`start`/`subflow_change`; `decision` now requires an explicit `wait: "no"`. `capture`/`store` were deliberately left unconstrained (no clear textual basis for restricting them). Verified against all real content under `agents/` — zero pre-existing violations. |
...
| ~~19~~ | ~~Fix B1~~ — **cancelled.** Confirmed by the repo owner (2026-08-06) that the duplicated `[[ input_variables_block ]]` render is intentional, not a bug. Do not implement. | N/A | N/A |
| ~~20~~ | ~~Fix B2~~ — **done (2026-08-06).** `FlowObjectBase.validate_semantics()`'s per-type invariants completed (`execute`, `final`, `say` restricted to the types `TYPE_SEMANTICS` documents them for; `decision` now requires explicit `wait: "no"`). | High | 13 new/updated schema tests (RED confirmed against pre-fix code, GREEN after); real content under `agents/` re-verified end-to-end via `compile_agent()` with zero new validation errors. |
...
| ~~**[B2]**~~ Incomplete per-node-type schema invariants let semantically invalid field combinations (e.g. `EXECUTE` on a `question` node) pass validation silently | — | — | **Fixed 2026-08-06**, roadmap phase 20 |
...
**Closed:** Open question 3 (invariant matrix for B2) was resolved 2026-08-06 — see decision log below.
...
**Context:** The repo owner reviewed the audit findings and corrected B1: the duplicated `[[ input_variables_block ]]` render in both `templates/*.j2` is intentional, not an accidental leftover. The owner then asked to proceed with completing B2's per-node-type schema invariants.

**Decision (B1):** Reclassified from a confirmed bug to "by design." Roadmap phase 19 cancelled. Left the templates untouched. §5.1's B1 row and the §11 risk row are struck through rather than deleted, so the audit record shows what was found and corrected rather than silently disappearing.

**Decision (B2):** Implemented the following per-type invariant matrix in `FlowObjectBase.validate_semantics()`, grounded strictly in `templates/system_prompt.md.j2`'s own `TYPE_SEMANTICS` prose (not speculative additions) and verified against every YAML file currently under `agents/` (read-only scan — zero pre-existing violations found, so the tightening is non-breaking today):

| Type | New constraint added | Textual basis |
|---|---|---|
| `message` | `final` may not be `'yes'` | Not documented as a closing node type |
| `question` | `execute` forbidden; `final` may not be `'yes'` | Only `action`/`terminal` are documented as executing tools; only `terminal` closes the interaction |
| `decision` | `wait` must be the explicit string `"no"` (no longer accepts an omitted/`None` value); `execute` forbidden; `final` may not be `'yes'` | Removes the sole type where `wait` was optional, matching every other non-`question` type; decision is documented as evaluate-then-route only |
| `action` | `say` must be empty | `TYPE_SEMANTICS`: "This state is non-conversational... the assistant's only valid output... is the tool call" |
| `action` | `final` may not be `'yes'` | Not documented as a closing node type |
| `registration` | `final` may not be `'yes'` | Not documented as a closing node type |
| `start` | `say` must be empty; `execute` forbidden | `TYPE_SEMANTICS`: "No user output; follow ROUTE immediately" |
| `subflow_change` | `say` must be empty; `execute` forbidden | Documented purely as a control transfer; real subflow_change nodes in `agents/` never set `say`/`execute` |

**Explicitly left unconstrained:** `capture` and `store` were NOT restricted to a type subset — the template's own field-level docs describe both generically across `STATE`/`HANDLER` without tying them to specific types, so adding a restriction there would be a speculative invariant not grounded in the DSL's documented semantics (CLAUDE.md §2). `registration`'s `say` was left unconstrained too — `TYPE_SEMANTICS` calls it "usually silent," not "always silent."

**Alternatives considered:**
- Also forbidding `capture`/`store` outside `question`/`action` — rejected; no clear textual basis, risk of breaking legitimate real content without justification.
- Leaving `decision`'s `wait` optional — rejected; the asymmetry served no purpose and produced inconsistent compiled `WAIT` lines for semantically identical nodes depending on authoring style.

**Consequences:** Any existing agent YAML (outside what was scanned in this repo's local `agents/`) that relies on the now-forbidden combinations will fail validation after this change — e.g. a `decision` node omitting `wait`, or a `start`/`subflow_change`/`action` node with non-empty `say`. This is a breaking schema change; SPEC.md §5.1's B2 row, EXPERTS.md's `@dsl-expert`, and `AGENT_CREATION_GUIDE.md` should be checked for accuracy against the new matrix.
